...
const { interpretMessage, PROMPT_VERSION, AI_MODEL } = require('../ai/interpreter');
...
const STATE_CHANGING = new Set([
  'REQUEST_JOB', 'CANCEL_JOB_REQUEST', 'ACCEPT_JOB', 'DECLINE_JOB', 'START_JOB',
  ...workflow.MILESTONE_EVENTS, 'END_JOB',
]);


async function getPendingAction(conversationId) {
  const r = await pool.query('SELECT pending_action FROM conversations WHERE id = $1', [conversationId]);
  return r.rows[0]?.pending_action ?? null;
}

async function setPendingAction(conversationId, action) {
  await pool.query('UPDATE conversations SET pending_action = $2 WHERE id = $1', [
    conversationId,
...
async function clearPendingAction(conversationId) {
  await pool.query('UPDATE conversations SET pending_action = NULL WHERE id = $1', [conversationId]);
}
...
async function reply({ conversationId, phone, body, buttons = null, listRows = null }) {
  ...
  try {
    let wamid;
    if (cleanButtons) wamid = await whatsapp.sendInteractiveButtons(phone, text, cleanButtons);
    else if (cleanRows) wamid = await whatsapp.sendInteractiveList(phone, text, 'Choose', cleanRows);
    ...
    await pool.query(`UPDATE messages SET delivery_status = 'FAILED' WHERE id = $1`, [messageId]);
...
async function storeInterpretation(messageId, candidate, processingStatus, errorMessage = null) {
  await pool.query(
...
async function loadInterpreterContext(driver, conversationId) {
  ...
  if (assignment) {
    ...
    const sR = await pool.query('SELECT summary_text FROM job_summaries WHERE job_id = $1', [assignment.job_id]);
    activeJobSummary = sR.rows[0]?.summary_text || null;
  }

  const pendingRequest = await workflow.getPendingRequest(driver);
  const availableJobs = assignment? [] : await workflow.listAvailableJobs(driver);
  ...
  return { activeJob, activeJobSummary, pendingRequest, availableJobs, recentMessages, pendingAction };
}


function fmtJobLine(j) {
  const qty = j.quantity != null ?`, ${j.quantity} ${j.unit ?? ''}`.trimEnd() : '';
...
function jobChoiceRows(jobs) {
  return jobs.map((j) => ({
    id: `CHOOSE_JOB:${j.id}`,
    title: j.job_number,
    description: `${j.pickup ?? '?'} → ${j.delivery ?? '?'}${j.quantity != null ? ` · ${j.quantity} ${j.unit ?? ''}` : ''}`.slice(0, 72),
  }));
}
...
async function transcribeVoiceMessage(messageId) {
  ...
  if (!transcript) {
    try {
      const { buffer, mimeType } = await whatsapp.downloadMedia(attachment.storage_key);
  ...
  await pool.query('UPDATE attachments SET transcription_text = $2 WHERE id = $1', [attachment.id, transcript]);
      await pool.query('UPDATE messages SET body_text = $2 WHERE id = $1', [messageId, ` ${transcript}`]);
  return transcript;
}
...
async function prepareDocumentContext(ctx, caption) {
  ...
  const documentType = POD_PATTERN.test(`${caption ?? ''} ${filenames}`) ? 'POD' : 'OTHER';
  ...
  if (!assignment) {
    ...
    if (jobs.length === 0) {
      return {
        linked: false,
      confirmation: 'Got the document, you don\'t have a job to attach it to right now, but the manager can still see it here.',
      };
    }
    ...
    return {
      ...
      jobRows: jobs.map((j) => ({
        id: `DOC_JOB:${j.id}`,
        title: j.job_number,
        description: `${j.pickup ?? '?'} → ${j.delivery ?? '?'} · ${j.current_status.replace(/_/g, ' ')}`.slice(0, 72),
      })),
    };
  }

  const { job } = await workflow.attachDocuments(d, { attachmentIds, documentType, caption, messageId, confidence: 1 });
  const confirmation = documentType === 'POD'
  ...
  return { linked: true, job, documentType, attachmentIds, caption, confirmation };
}
...
async function handleInboundImage(ctx, caption) {
  ...
  const att = await pool.query(
    `SELECT id, public_url FROM attachments
      WHERE message_id = $1 AND attachment_type = 'IMAGE'
  ...
  if (!visionResult) {
    const stageDoc = await handleStageDocument(ctx, { attachment, imageUrl, stage: null, caption });
    return stageDoc;
  }
  ...
  if (['ACCIDENT', 'BREAKDOWN', 'DAMAGE'].includes(visionResult.scene_class)) {
    ...
    const { job, incidentId, incidentType, severity, alreadyReported } = await workflow.reportIncident(d, {
      incidentType: visionResult.scene_class,
      severity: visionResult.severity_hint,
      description: visionResult.description || `${visionResult.scene_class} detected in driver photo`,
      data: { source: 'VISION' },
      messageId,
      confidence: 1,
      imageUrl,
  ...
  if (['PICKUP_PAPER', 'DELIVERY_PAPER', 'POD'].includes(visionResult.scene_class)) {
    return await handleStageDocument(ctx, {
      attachment,
      imageUrl,
      stage: visionResult.scene_class === 'PICKUP_PAPER' ? 'PICKUP' : 'DELIVERY',
      caption,
    });
  }
...
async function handleStageDocument(ctx, { attachment, imageUrl, stage: hintedStage, caption }) {
  const { d, messageId } = ctx;
  ...
  const stage = resolveDocumentStage(job, hintedStage);
  const expectedCode = stage === 'PICKUP' ? job.pickup_code : job.delivery_code;
  if (!expectedCode) return null;

  const extraction = await documentCode.extractDocumentCodes(attachment.id, caption);

  if (extraction.outcome === documentCode.OUTCOME.TECH_FAIL) {
    console.error(`Document code extraction failed for ${job.job_number}:`, extraction.error);
  ...
  if (extraction.outcome === documentCode.OUTCOME.UNCLEAR) {
    await workflow.recordEvent(pool, {
      ...
      eventType: 'DOCUMENT_VERIFICATION_FAILED',
      status: 'REVIEW_REQUIRED',
      data: { stage, attachment_id: attachment.id, failure: 'UNREADABLE', notes: extraction.notes },
    });
    return await reply({
      ...ctx,
  ...
  const result = await workflow.verifyDocument(d, {
    jobId: job.id, stage, extraction,
    attachmentId: attachment.id, messageId, imageUrl,
  });
  ...
  if (result.needsStartConfirmation) {
    await setPendingAction(ctx.conversationId, { type: 'AWAITING_START_CONFIRM', candidate: null });
    return await reply({
      ...ctx,
  ...
  if (result.needsDelayReason) {
    ...
    await setPendingAction(ctx.conversationId, { type: 'AWAITING_DELAY_REASON', stage: dl.stage });
    return await reply({
      ...ctx,
...
function resolveDocumentStage(job, hintedStage) {
  if (job.pickup_verified_at && !job.delivery_verified_at) return 'DELIVERY';
  if (!job.pickup_verified_at && !job.delivery_verified_at) {
    const rank = workflow.stateMachine.rankOf(job.current_status) ?? 0;
    if (rank >= workflow.stateMachine.STAGE_RANK.DEPARTED) return 'DELIVERY';
    return 'PICKUP';
  }
...
async function applySafetyCheck(ctx, pending, { lat = null, lng = null, backup = null }) {
  ...
  if (backup) {
    await pool.query(
      `UPDATE incidents SET backup_vehicle = $2 WHERE id = $1`,
      [pending.incidentId, backup],
    );
  }
  ...
  if (!incident) {
    await clearPendingAction(ctx.conversationId);
    return reply({ ...ctx, body: "That incident is already closed, so no further safety details are needed." });
  }
  if (incident.location_lat == null) {
    await setPendingAction(ctx.conversationId, { ...pending, type: 'AWAITING_LOCATION' });
    return reply({
      ...ctx,
  ...
  if (!incident.backup_vehicle) {
    await setPendingAction(ctx.conversationId, { ...pending, type: 'AWAITING_BACKUP' });
    return reply({
      ...ctx,
...
async function describeBlocker(driver) {
  try {
    ...
    if (progress.stage === 'DRIVER_SUBMITTED_COMPLETION' || progress.stage === 'MANAGER_REVIEW') {
...
async function processInboundCore({ driver, conversationId, messageId, messageType, bodyText, buttonId }) {
  const d = { id: driver.driver_id ?? driver.id, name: driver.name, organization_id: driver.organization_id, vehicle_type: driver.vehicle_type ?? null };
  ...
  try {
    ...
    if (messageType === 'LOCATION') {
      ...
      if (['AWAITING_LOCATION', 'AWAITING_BACKUP'].includes(pendingLoc?.type)) {
        ...
        if (!m) {
      return await reply({ ...ctx, body: 'Hmm, I couldn\'t read that location, use the attachment menu in WhatsApp and choose Location to share your position.' });
        }
      ...
      return await reply({ ...ctx, body: 'Got it, the manager can see where you are now.' });
      }
    ...
    if (messageType === 'IMAGE' || messageType === 'DOCUMENT') {
      ...
      return await reply({ ...ctx, body: 'Which job is this for? Tap below', listRows: doc.jobRows });
      }
      if (doc) {
        ctx.document = doc;
    ...
    if (!text) {
      return await reply({ ...ctx, body: 'Hmm, that came through empty on my side, can you type it out? (e.g. "loaded 18 pallets")' });
    }

    const pending = await getPendingAction(conversationId);
    if (['AWAITING_LOCATION', 'AWAITING_BACKUP'].includes(pending?.type)) {
      return await reply({ ...ctx, body: 'Almost done, share your current location using the attachment menu in WhatsApp, or tap one of the backup options above. ' });
    }
    if (pending?.type === 'AWAITING_CANCEL_REASON') {
      ...
      return await reply({ ...ctx, body: 'No worries, your job stays as it is. ' });
      }
    ...
    if (pending?.type === 'AWAITING_DELAY_REASON') {
      ...
      await clearPendingAction(conversationId);
      const rec = await workflow.recordStageDelay(d, stage, text.slice(0, 500), messageId);
      const lines = [
        rec.alreadyRecorded
    ...
    if (!screen.safe) {
      await storeInterpretation(
        messageId,
        { intent: 'BLOCKED_INJECTION', events: [], job_reference: null, extracted_data: { score: screen.score }, confidence: 1 },
        'REQUIRES_REVIEW',
      ...
      return await reply({ ...ctx, body: "Hmm, that message didn't come through right, just send it to me in your own words, like a normal job update." });
    }
    ...
    if (pending?.type === 'AWAITING_ACCEPT_DECLINE' && candidate.intent === 'CONFIRM_EVENT') {
      await clearPendingAction(conversationId);
      return await applyCandidate(
        { intent: 'ACCEPT_JOB', job_reference: null, extracted_data: {}, confidence: 1, events: [] },
        ctx,
        { confirmed: true }
      );
    }

    if (pending?.type === 'AWAITING_APPLY' && (candidate.intent === 'CONFIRM_EVENT' || candidate.intent === 'REQUEST_JOB')) {
      const inviteJobId = pending.job_id;
      await clearPendingAction(conversationId);
      return await submitApplyRequest(ctx, inviteJobId);
    }

    if (pending?.type === 'AWAITING_START_CONFIRM' && candidate.intent === 'CONFIRM_EVENT') {
      await clearPendingAction(conversationId);
    ...
    if (pending?.type === 'AWAITING_CONFIRMATION') {
      ...
      if (candidate.intent === 'CORRECT_EVENT') {
        const merged = {
          ...pending.candidate,
          extracted_data: { ...pending.candidate.extracted_data, ...candidate.extracted_data },
        };
        await setPendingAction(conversationId, { type: 'AWAITING_CONFIRMATION', candidate: merged });
    ...
    if (pending?.type === 'AWAITING_FREEFORM_ANSWER') {
      await clearPendingAction(conversationId);
      if (['CONFIRM_EVENT', 'CORRECT_EVENT', 'UNKNOWN'].includes(candidate.intent)) {
        const blocker = await describeBlocker(d);
        const human = await voiceOf(ctx, [
          `You asked the driver: "${pending.question}"`,
          `They replied: "${text}"`,
          'Answer their reply directly and give the one next step that follows from it.',
          blocker ? `What the trip is waiting on right now: ${blocker}` : null,
    ...
    if (err instanceof workflow.WorkflowError) {
      const VERBATIM = ['PROTECTED_MILESTONE', 'VERIFICATION_REQUIRED', 'JOB_FROZEN', 'NOT_DELIVERED', 'START_TOO_EARLY', 'START_CONFIRMATION_REQUIRED', 'NO_DELAY'];
      ...
      const human = await voiceOf(ctx, `The driver's update could not be applied. Reason: "${err.message}". Explain warmly and briefly what is missing and the one thing they can do next. Do not claim anything was recorded. Do not use emojis or em dashes.`);
      return await reply({ ...ctx, body: human ?? err.message });
    }
    console.error('Bot processing failed:', err);
    return await reply({ ...ctx, body: 'Something went wrong on my side. Please try again in a moment.' }).catch(() => {});
  }
}
...
async function processInbound(args) {
  ...
  summarizer.tickAfterMessage(driverId, args.conversationId)
...
async function submitApplyRequest(ctx, jobId) {
  const { d, messageId } = ctx;
  const job = (await pool.query(
    'SELECT id, organization_id, job_number, current_status FROM jobs WHERE id = $1',
    [jobId]
  )).rows[0];
  if (!job) return await reply({ ...ctx, body: 'That job is no longer open, say "jobs" and I\'ll show what\'s available.' });
  try {
    const { job: requested } = await workflow.requestJob(d, job, messageId, 1);
    return await reply({ ...ctx, body: `Done, I've asked the manager about *${requested.job_number}*. I'll ping you the moment it's approved!` });
  } catch (err) {
    if (err.code === 'DUPLICATE_REQUEST') {
      return await reply({ ...ctx, body: `You're already in the running for *${job.job_number}* — just waiting on the manager now.` });
    }
    if (err.code === 'DRIVER_BUSY') {
      return await reply({ ...ctx, body: 'You\'re already tied to another job — finish that one and I\'ll line up the next.' });
    }
    return await reply({ ...ctx, body: `Couldn't put your name in for *${job.job_number}* right now, give it another go in a bit.` });
  }
}
...
async function handleButton(buttonId, ctx) {
  ...
  if (buttonId === 'APPLY_JOB') {
    ...
    await clearPendingAction(conversationId);
    if (!jobId) return await reply({ ...ctx, body: 'That invite expired on me, say "jobs" and I\'ll show what\'s open.' });
    return await submitApplyRequest(ctx, jobId);
  }

  if (buttonId === 'ACCEPT_JOB' || buttonId === 'DECLINE_JOB') {
    const fn = buttonId === 'ACCEPT_JOB' ? workflow.acceptJob : workflow.declineJob;
    const { job } = await fn(d, messageId, 1);
    await clearPendingAction(conversationId);
    if (buttonId === 'ACCEPT_JOB') {
      summarizer.ensureSummaryRow(job.id, d.id).catch((err) => console.error('Summary row create failed:', err.message));
    }
    return await reply({
      ...ctx,
      body: buttonId === 'ACCEPT_JOB'
  ...
  if (buttonId === 'CONFIRM_YES' || buttonId === 'CONFIRM_NO') {
    const pending = await getPendingAction(conversationId);
    if (pending?.type !== 'AWAITING_CONFIRMATION') {
      return await reply({ ...ctx, body: 'That one expired on me, send the update again and I\'ll pick it up.' });
    }
    await clearPendingAction(conversationId);
    if (buttonId === 'CONFIRM_NO') {
      return await reply({ ...ctx, body: 'No problem, scrapped it, nothing changed. ' });
    }
    return await applyCandidate(pending.candidate, ctx, { confirmed: true });
  }
  ...
  if (buttonId.startsWith('CANCEL_REASON:')) {
    const pending = await getPendingAction(conversationId);
    if (pending?.type !== 'AWAITING_CANCEL_REASON') {
      return await reply({ ...ctx, body: 'There\'s no cancellation request open right now, all good. ' });
    }
    const labels = {
  ...
  if (buttonId === 'END_JOB_CONFIRM_YES' || buttonId === 'END_JOB_CONFIRM_NO') {
    if (buttonId === 'END_JOB_CONFIRM_NO') {
      return await reply({ ...ctx, body: 'Got it, trip stays open. Keep the updates coming! ' });
    }
    return await applyCandidate({ intent: 'END_JOB', job_reference: null, extracted_data: {}, confidence: 1, events: [] }, ctx, { confirmed: true });
  }

  if (buttonId === 'BACKUP_NEEDED' || buttonId === 'BACKUP_NOT_NEEDED') {
    const pending = await getPendingAction(conversationId);
    if (!['AWAITING_LOCATION', 'AWAITING_BACKUP'].includes(pending?.type)) {
      return await reply({ ...ctx, body: 'There is no safety check in progress right now.' });
    }
    return await applySafetyCheck(ctx, pending, { backup: buttonId === 'BACKUP_NEEDED' ? 'REQUESTED' : 'DECLINED' });
  }
  ...
  if (buttonId === 'GAP_START_YES' || buttonId === 'GAP_START_NO') {
    ...
    if (buttonId === 'GAP_START_NO') {
      return await reply({ ...ctx, body: 'No problem Nothing logged, just say the word when you want to start the trip.' });
    }
    return await confirmStartAndContinue(ctx, pending.candidate ?? null);
  }
  ...
  if (buttonId === 'CONFIRM_START_JOB' || buttonId === 'START_NOT_YET') {
    ...
    if (buttonId === 'START_NOT_YET') {
      return await reply({ ...ctx, body: 'No problem, nothing changed. Just say "start job" here when you actually roll.' });
    }
    return await confirmStartAndContinue(ctx, pending.candidate ?? null);
  }
  ...
  if (buttonId.startsWith('CHOOSE_JOB:')) {
    ...
    const job = (await pool.query('SELECT * FROM jobs WHERE id = $1', [jobId])).rows[0];
      if (!job) return await reply({ ...ctx, body: 'Ah, that one\'s gone already, say "jobs" and I\'ll show you what\'s open now.' });
    await clearPendingAction(conversationId);
    const { job: requested } = await workflow.requestJob(d, job, messageId, 1);
    return await reply({
      ...ctx,
  ...
  if (buttonId.startsWith('DOC_JOB:')) {
    ...
    if (pending?.type !== 'AWAITING_DOC_JOB_CHOICE') {
      return await reply({ ...ctx, body: 'That document request expired, mind sending the photo again ? ' });
    }
    ...
    return await reply({
      ...
      return await reply({ ...ctx, body: 'Hmm, that button\'s from an old message, what\'s happening with the trip?' });
}
...
async function routeCandidate(candidate, ctx) {
  ...
  if (
    ...
    !/(did|done|before|previous|prev|past|history|earlier|completed|finished)/i.test(rawText)
  ) {
    return await applyCandidate({ ...candidate, intent: 'SHOW_AVAILABLE_JOBS' }, ctx, { confirmed: false });
  }

  if (candidate.intent === 'END_JOB') {
    const incident = await findOpenSafetyIncident(d.id);
    if (incident) return await beginSafetyFlow(ctx, incident, 'Driver asked to end the trip after an incident');
  ...
  if (candidate.confidence < CLARIFY_MIN && candidate.intent !== 'UNKNOWN') {
    await setPendingAction(conversationId, { type: 'AWAITING_CLARIFICATION', candidate });
    return await reply({
      ...ctx,
      body: candidate.clarification_question
  ...
  if (STATE_CHANGING.has(candidate.intent) && (candidate.needs_confirmation || candidate.confidence < AUTO_APPLY_MIN)) {
    await setPendingAction(conversationId, { type: 'AWAITING_CONFIRMATION', candidate });
...
async function applyCandidate(candidate, ctx, { confirmed }) {
  ...
  const confidence = confirmed? Math.max(candidate.confidence, 0.9) : candidate.confidence;
  ...
  switch (candidate.intent) {
    ...
    case 'SHOW_AVAILABLE_JOBS': {
      ...
      const jobs = await vehicleMatch.filterJobsForDriver(d, await workflow.listAvailableJobs(d));
      if (jobs.length === 0) {
        const fit = d.vehicle_type ? ` that suits your ${d.vehicle_type}` : '';
        return await reply({ ...ctx, body: `Nothing open${fit} right now, I'll message you the second a suitable job drops.` });
      }
      await setPendingAction(ctx.conversationId, { type: 'AWAITING_JOB_CHOICE', jobIds: jobs.map((j) => j.id) });
      return await reply({
        ...ctx,
    ...
    case 'REQUEST_JOB': {
      if (!candidate.job_reference) {
        ...
        if (jobs.length === 0) {
          const fit = d.vehicle_type ? ` that suits your ${d.vehicle_type}` : '';
          return await reply({ ...ctx, body: `Nothing open${fit} to request right now, I'll ping you as soon as a suitable job drops.` });
        }
        await setPendingAction(ctx.conversationId, { type: 'AWAITING_JOB_CHOICE', jobIds: jobs.map((j) => j.id) });
      return await reply({ ...ctx, body: 'Which one do you want? Tap below', listRows: jobChoiceRows(jobs) });
      }
      const job = await workflow.getJobByNumber(pool, d.organization_id, candidate.job_reference);
      if (!job) return await reply({ ...ctx, body: `Hmm, I can't find a job matching "${candidate.job_reference}", say "jobs" and I'll list what's open.` });
      const { job: requested } = await workflow.requestJob(d, job, messageId, confidence);
      return await reply({
        ...ctx,
      body: `Done, I've asked the manager about *${requested.job_number}*. I'll ping you the moment it's approved!`,
      });
    ...
    case 'CANCEL_JOB_REQUEST': {
      ...
      return await reply({ ...ctx, body: `Done, request for *${job.job_number}* withdrawn. ` });
    }

    case 'ACCEPT_JOB': {
      const { job } = await workflow.acceptJob(d, messageId, confidence);
      summarizer.ensureSummaryRow(job.id, d.id).catch((err) => console.error('Summary row create failed:', err.message));
      return await reply({ ...ctx, body: `You're on *${job.job_number}*! Say "start ${job.job_number}" when you're ready to roll. ` });
    }

    case 'DECLINE_JOB': {
      const { job } = await workflow.declineJob(d, messageId, confidence);
      return await reply({ ...ctx, body: `No worries, *${job.job_number}* goes back to the pool. ` });
    }

    case 'START_JOB': {
      const { job, alreadyStarted } = await workflow.startJob(d, messageId, confidence);
      if (alreadyStarted) {
        if (data.correction) {
          return await reply({
            ...ctx,
      ...
      const lines = [`*${job.job_number}* is rolling. Keep me posted as you go: "reached pickup", "on my way", "at delivery". Drive safe.`];
      const cont = await workflow.applyPendingVerifications(d, messageId);
      await appendPendingContinuation(lines, ctx, cont);
      return await reply({ ...ctx, body: lines.join(' ') });
    }
    ...
    case 'ASK_PROGRESS': {
      ...
      if (!progress) {
        return await reply({ ...ctx, body: 'You are not on a job right now. Say "jobs" and I will show you what is open.' });
      }
      const next = progress.nextStep ? ` ${progress.nextStep}` : '';
      const pendingBits = (progress.pending ?? []).map((p) => p.message);
      const pendingText = pendingBits.length > 0
    ...
    case 'DELAY_REPORTED': {
      const { job } = await workflow.reportDelay(d, data, messageId, confidence);
    ...
    case 'INCIDENT_REPORTED': {
      const map = {
        ...
        INCIDENT_REPORTED: { type: data.incident_type, eventType: 'INCIDENT_REPORTED' },
      }[candidate.intent];
      const description = data.description ?? data.reason ?? `${candidate.intent.replace(/_/g, ' ').toLowerCase()} reported by driver`;
      const { job, incidentType, severity, alreadyReported } = await workflow.reportIncident(d, {
      ...
      const emergencyLine = severity === 'CRITICAL'
    ...
    case 'CANCELLATION_REQUESTED': {
      if (!data.reason) {
        await setPendingAction(ctx.conversationId, { type: 'AWAITING_CANCEL_REASON' });
        return await reply({
          ...ctx,
    ...
    case 'END_JOB': {
      const { job, alreadyRequested } = await workflow.endJob(d, messageId, confidence);
      if (alreadyRequested) {
        return await reply({
          ...ctx,
      ...
      const needsPod = /pod/i.test(job.special_instructions ?? '') && !(await workflow.hasPodDocument(job.id));
    ...
    case 'DOCUMENT_RECEIVED': {
      ...
      return await reply({ ...ctx, body: 'Send the photo or document right here and I\'ll attach it to your job. ' });
    }
    ...
    case 'ASK_IDENTITY': {
      return await reply({
        ...ctx,
      body: "I'm the DevCax Logistics internal AI agent. I work with the dispatch team to track trips, send me progress updates, photos, voice notes or questions and I'll keep the manager informed.",
      });
    }
    ...
    case 'ASK_HISTORY': {
      ...
      return await reply({ ...ctx, body: 'No completed trips on record yet, your first one is still ahead of you! Say "jobs" when you\'re ready. ' });
      }
      ...
      return await reply({ ...ctx, body: `Here are your recent trips:\n${lines.join('\n')}` });
    }
    ...
    default: {
      ...
      const situation = [
        ...
        blocker ? `Only mention this if the driver clearly asked what to do next: ${blocker}` : null,
        'Never claim an action was taken or recorded. Do not use emojis or em dashes.',
      ].filter(Boolean).join(' ');
      const human = await voiceOf(ctx, situation);
      const body = human ?? candidate.clarification_question ?? 'Hmm, I didn\'t quite catch that, can you say it again in different words?';
...
async function applyMilestoneCandidate(candidate, ctx, { confirmed }) {
  ...
  let result;
  try {
    result = await workflow.applyMilestone(d, target, data, messageId, confidence);
  } catch (err) {
    if (err instanceof workflow.WorkflowError && err.code === 'START_CONFIRMATION_REQUIRED') {
      await setPendingAction(ctx.conversationId, { type: 'AWAITING_START_CONFIRM', candidate });
      const assignment = await workflow.getDriverAssignment(pool, d.id);
      const jobRef = assignment?.job_number ?? candidate.job_reference ?? 'your job';
      return await reply({
        ...ctx,
  ...
  const extra = newStatus === 'DELIVERED_WITH_EXCEPTION'
...
async function appendPendingContinuation(lines, ctx, cont) {
  const applied = [...new Set((cont?.advances ?? []).flatMap((a) => a.applied ?? []))];
  if (applied.length > 0) {
    lines.push(`Your verified paperwork is applied now: the job is marked as ${joinList(applied.map((s) => workflow.stateMachine.label(s)))}.`);
  }
  if (cont?.needsDelayReason) {
    const dl = cont.needsDelayReason;
    await setPendingAction(ctx.conversationId, { type: 'AWAITING_DELAY_REASON', stage: dl.stage });
    lines.push(`The ${dl.stage.toLowerCase()} was about ${dl.minutesLate} min late (scheduled ${workflow.fmtWhen(dl.scheduledAt)}). What caused the delay?`);
  }
}
...
async function confirmStartAndContinue(ctx, candidate = null) {
  ...
  if (candidate && MILESTONE_SET.has(candidate.intent)) {
    ...
    const freshJob = (await pool.query('SELECT * FROM jobs WHERE id = $1', [job.id])).rows[0];
    const nowRank = await workflow.currentMilestoneRank(pool, freshJob);
    if (targetRank > nowRank) {
      if (workflow.stateMachine.requiresDocumentVerification(candidate.intent)) {
        lines.push(workflow.stateMachine.verificationStageFor(candidate.intent) === 'PICKUP'
          ? 'To mark it as loaded, send a photo of the pickup document.'
          : 'To mark it as delivered, send a photo of the delivery document.');
      } else {
        const r = await workflow.applyMilestone(d, candidate.intent, candidate.extracted_data ?? {}, messageId, 1);
        if (!r.noop) {
          lines.push(`Also logged: the job is now marked as ${workflow.stateMachine.label(r.newStatus)}.`);
        }
      }
    }
  }
...
async function deliverOffer({ job, driver, body, buttons, pendingType = 'AWAITING_ACCEPT_DECLINE' }) {
...
async function sendJobApplyInvite({ job, driver }) {
  const when = job.pickup_at
    ? new Date(job.pickup_at).toLocaleString('en-GB', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })
    : null;
  const route = [job.pickup, job.delivery].filter(Boolean).join(' to ');
  const quantity = job.quantity != null ? `, ${job.quantity} ${job.unit ?? ''}`.trimEnd() : '';
  const vehicle = driver.vehicle_type ? ` that suits your ${driver.vehicle_type}` : '';
...
async function sendRequestRejected({ job, driver }) {
      return await notifyDriver({ job, driver, body: `Ah, *${job.job_number}* went another way this time Say "jobs" and I'll show you what else is open.` });
}
...
async function sendCancellationDecision({ job, driver, action }) {
  ...
  }[action] ?? `Update on your cancellation request for *${job.job_number}*: ${action}.`;
